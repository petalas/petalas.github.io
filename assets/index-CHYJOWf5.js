(function(){let e=document.createElement(`link`).relList;if(e&&e.supports&&e.supports(`modulepreload`))return;for(let e of document.querySelectorAll(`link[rel="modulepreload"]`))n(e);new MutationObserver(e=>{for(let t of e)if(t.type===`childList`)for(let e of t.addedNodes)e.tagName===`LINK`&&e.rel===`modulepreload`&&n(e)}).observe(document,{childList:!0,subtree:!0});function t(e){let t={};return e.integrity&&(t.integrity=e.integrity),e.referrerPolicy&&(t.referrerPolicy=e.referrerPolicy),e.crossOrigin===`use-credentials`?t.credentials=`include`:e.crossOrigin===`anonymous`?t.credentials=`omit`:t.credentials=`same-origin`,t}function n(e){if(e.ep)return;e.ep=!0;let n=t(e);fetch(e.href,n)}})();var e=`<svg viewBox="0 0 100 100">
    ...
    <svg viewBox="0 0 846.66 846.66">
      ...
      />
    </svg>`,n=`<svg x="0px" y="0px" viewBox="0 0 100 100">
      <polygon points="2.5,30 10.1,22.5 50,62.4 89.9,22.5 97.5,30 50,77.5" />
    </svg>`,r=`<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 32 32">
  <path
...
</svg>`,i=`<svg viewBox="0 0 64 64" xmlns="http://www.w3.org/2000/svg">
    ...
    </svg>`,a=`<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100">
    ...
    </svg>`,s=`<svg {...props} viewBox="0 0 50 50">
    ...
    </svg>`,l=`<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 64 64">
    ...
    </svg>`,d=`<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 40 40">
    ...
    </svg>`,f=`<svg viewBox="0 0 16 16" xmlns="http://www.w3.org/2000/svg">
    ...
    </svg>`;function p(n){n&&(n.innerHTML=`
  ...
  `)}function m(e){e&&(e.innerHTML=`
    ${b.map(y).join(``)}
    ${h()}
  `)}var h=()=>{let e=b.map(e=>({"@type":`WorkPosition`,name:e.title,employer:{"@type":`Organization`,name:e.name},employmentType:`FULL_TIME`,startDate:_(e.timeframe),endDate:v(e.timeframe),description:e.description.replace(/<[^>]*>/g,``).trim()}));return`
    ...
    <\/script>
  `},g=e=>{let t={Jan:0,Feb:1,Mar:2,Apr:3,May:4,Jun:5,Jul:6,Aug:7,Sep:8,Oct:9,Nov:10,Dec:11},n=e.match(/^(\w+)\s+(\d{4})$/);if(!n)return``;let[,r,i]=n,a=t[r],o=parseInt(i,10);return a===void 0||isNaN(o)?``:new Date(o,a,1).toISOString().split(`T`)[0]},_=e=>{let t=e.match(/^(\w+\s+\d{4})/);return t?g(t[1]):``},v=e=>{if(e.includes(`Present`))return``;let t=e.match(/–\s*(\w+\s+\d{4})$/);return t?g(t[1]):``},y=e=>`
  ...
  `,b=[{title:`Senior Frontend Developer`,name:`Eeze (Game Innovation)`,timeframe:`Sep 2025 – Present`,description:`
    ...
    `},{title:`Founder & Lead Engineer`,name:`DailyGoal.fit`,timeframe:`Oct 2024 – Present`,description:`
      <ul class="m-2 p-2 list-disc list-outside">
    ...
    `}];function x(e){e&&(e.innerHTML=`© ${new Date().getFullYear()}, Nick Petalas`)}function S(e){e&&(e.innerHTML=`
    ...
    <section class="col-span-1 lg:col-span-2 xl:col-span-3" itemscope itemtype="https://schema.org/Person">${C()}</section>
  `)}var C=()=>`
  ...
  `;function w(e){if(!e)return;e.innerHTML=`
    <div class="nav-container">
      ...
      <ul id="nav-list" class="nav-list hidden flex-col fixed top-16 left-0 right-0 bg-latte-crust dark:bg-mocha-crust shadow-lg border-b border-latte-surface0 dark:border-mocha-surface0 md:relative md:flex md:flex-row md:h-auto md:top-0 md:left-0 md:right-0 md:bg-transparent md:dark:bg-transparent md:shadow-none md:border-none">
        <li class="nav-item"><a href="#about">${o}<span>About</span></a></li>
        <li class="nav-item"><a href="#experience">${i}<span>Experience</span></a></li>
        <li class="nav-item"><a href="#projects">${d}<span>Projects</span></a></li>
        <li class="nav-item"><a href="#links">${l}<span>Links</span></a></li>
        <li class="nav-item"><a href="/Nick_Petalas_CV.pdf" download="Nick_Petalas_CV.pdf">${t}<span>CV.pdf</span></a></li>
      </ul>
      <div class="mobile-controls">
        <div class="theme-toggle-container">
          <button id="theme-button" class="theme-toggle-btn" aria-label="Toggle dark mode">${f}</button>
        </div>
        <div id="nav-toggle" class="nav-toggle-button">${n}</div>
      </div>
    </div>
  `;let r=document.querySelector(`#nav-list`),a=document.querySelector(`#nav-toggle`),s=()=>{window.requestAnimationFrame(()=>{r?.classList.contains(`hidden`)?p():c()})},c=()=>{r?.classList?.add(`hidden`),r?.classList?.remove(`flex`),a?.classList?.remove(`rotate-180`)},p=()=>{r?.classList?.remove(`hidden`),r?.classList?.add(`flex`),a?.classList?.add(`rotate-180`)};a.onclick=()=>s();let m=r?.children;for(let e=0;e<(m?.length??0);e++)m?.[e]?.addEventListener(`click`,()=>c());let h=localStorage.getItem(`darkMode`);h===`true`?document.body.classList.add(`dark`):h===`false`&&document.body.classList.remove(`dark`);let g=document.querySelector(`#theme-button`);g&&(document.body.classList.contains(`dark`)?g.innerHTML=f:g.innerHTML=u,g.onclick=()=>{document.body.classList.toggle(`dark`),document.body.classList.contains(`dark`)?(g.innerHTML=f,localStorage.setItem(`darkMode`,`true`)):(g.innerHTML=u,localStorage.setItem(`darkMode`,`false`))})}function T(e){e&&(e.innerHTML=`
    ${M.map(j).join(``)}
    ${D()}
  `,E())}function E(){document.querySelectorAll(`.screenshot-carousel`).forEach(e=>{let t=e.querySelector(`.carousel-track`),n=e.querySelectorAll(`.carousel-dot`),r=e.querySelector(`.carousel-prev`),i=e.querySelector(`.carousel-next`);if(!t)return;let a=t.querySelectorAll(`.carousel-item`),o=0,s=null,c=()=>window.innerWidth>=1024?4:window.innerWidth>=768?3:window.innerWidth>=640?2:1,l=()=>Math.max(0,a.length-c()),u=e=>{o=Math.max(0,Math.min(e,l()));let n=a[o];n&&t.scrollTo({left:n.offsetLeft-t.offsetLeft,behavior:`smooth`}),d()},d=()=>{n.forEach((e,t)=>{let n=t===o;e.classList.toggle(`bg-latte-blue`,n),e.classList.toggle(`dark:bg-mocha-blue`,n),e.classList.toggle(`bg-latte-overlay0`,!n),e.classList.toggle(`dark:bg-mocha-overlay0`,!n),e.classList.toggle(`scale-125`,n)})},f=()=>{p(),s=setInterval(()=>{let e=o+1;u(e>l()?0:e)},4e3)},p=()=>{s&&=(clearInterval(s),null)};r?.addEventListener(`click`,()=>{p(),u(o<=0?l():o-1),f()}),i?.addEventListener(`click`,()=>{p(),u(o>=l()?0:o+1),f()}),n.forEach((e,t)=>{e.addEventListener(`click`,()=>{p(),u(t),f()})}),e.addEventListener(`mouseenter`,p),e.addEventListener(`mouseleave`,f);let m=null;t.addEventListener(`scroll`,()=>{m&&clearTimeout(m),m=setTimeout(()=>{let e=t.scrollLeft,n=0,r=1/0;a.forEach((i,a)=>{let o=Math.abs(i.offsetLeft-t.offsetLeft-e);o<r&&(r=o,n=a)}),o=n,d()},50)}),window.addEventListener(`resize`,()=>{u(Math.min(o,l()))}),d(),f()})}var D=()=>{let e=M.map(e=>({"@type":`SoftwareApplication`,name:e.name,url:e.link??``,image:e.img?`https://petalas.dev/${e.img}`:``,description:e.description.replace(/<[^>]*>/g,``).trim(),author:{"@type":`Person`,name:`Nick Petalas`},applicationCategory:`WebApplication`}));return`
    <script type="application/ld+json">
    {
      "@context": "https://schema.org",
      "@type": "Person",
    ...
    <\/script>
  `},O=`https://apps.apple.com/gb/app/dailygoal-fitness-nutrition/id6758465266`,k=[{file:`home`,label:`Know today's target`},{file:`diary`,label:`Log food in seconds`},{file:`progress`,label:`See the real trend`},{file:`workouts`,label:`Execute the plan`},{file:`exercise-library`,label:`Pick the movement`},{file:`preferences`,label:`Connect Apple Health`},{file:`goal-settings`,label:`Tune your targets`},{file:`widgets-home`,label:`Keep goals on the Home Screen`},{file:`widgets-lock`,label:`Glance from the Lock Screen`},{file:`leaderboard-tabs`,label:`Compete on your terms`}],A=e=>`
    <article class="project md:col-span-2">
      <h3 class="p-2 bg-latte-crust dark:bg-mocha-crust">${`<a href="${e.link}" target="_blank" rel="noopener noreferrer">${e.name}</a>`}</h3>
      ...
      <div class="screenshot-carousel relative select-none">
        <div class="carousel-track flex gap-3 overflow-x-auto scroll-smooth p-4 snap-x snap-mandatory scrollbar-hide">
          ${k.map(({file:e,label:t})=>`
            <div class="carousel-item snap-start shrink-0">
              <img
                src="images/dailygoal-screenshots/${e}.png"
                alt="DailyGoal app — ${t}"
        ...
        <div class="flex justify-center gap-2 pb-3 pt-1">
          ${k.map((e,t)=>`<button class="carousel-dot w-2 h-2 rounded-full bg-latte-overlay0 dark:bg-mocha-overlay0 transition-all duration-200 cursor-pointer hover:scale-150" aria-label="Go to screenshot ${t+1}"></button>`).join(``)}
        </div>
      </div>

      <div class="flex justify-center py-3">
        <a href="${O}" target="_blank" rel="noopener noreferrer" class="app-store-badge inline-flex items-center gap-2 px-5 py-2.5 bg-black text-white rounded-xl hover:opacity-80 transition-opacity no-underline">
      ...
      <div class="text-sm p-4 pt-0">${e.description}</div>
    </article>
  `,j=e=>e.name===`DailyGoal.fit`?A(e):`
  ...
  `,M=[{name:`DailyGoal.fit`,link:`https://www.dailygoal.fit`,img:`images/dailygoal.avif`,description:`
    ...
    `}];function N(){let e=document.querySelector(`#app`);if(!e){console.error(`App element not found`);return}e.innerHTML=`
    <div class="wrapper">
  ...
  `;let t=document.querySelector(`#nav`),n=document.querySelector(`#about`),r=document.querySelector(`#experience`),i=document.querySelector(`#projects`),a=document.querySelector(`#links`),o=document.querySelector(`#footer`);t&&w(t),n&&S(n),r&&m(r),i&&T(i),a&&p(a),o&&x(o)}document.readyState===`loading`?document.addEventListener(`DOMContentLoaded`,N):N();